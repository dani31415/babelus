...
import { openDatabase, CreateObjectStore } from './indexed-db';
import { createTransaction, optionsGenerator, validateBeforeTransaction } from './utils';
import { DBConfig, Key, RequestEvent, ObjectStoreMeta, DBMode } from './indexed-db.meta';
...
export class NgxIndexedDBService<T = any> {
  ...
  constructor(private dbConfig: DBConfig) {
    ...
    this.isBrowser = true; // isPlatformBrowser(this.platformId);
    if (this.isBrowser) {
      this.indexedDB =
        window.indexedDB ||
